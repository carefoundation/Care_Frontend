...
const medicineTypes = [
  'Allopathic', 'Ayurvedic', 'Homeopathic', 'Unani', 'Generic Medicines',
  'Surgical Equipment', 'Medical Supplies', 'Other'
];
...
export default function CreateMedicinePage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: string) => {
    if (e.target.files) {
      if (field === 'pharmacyImages') {
        setFiles(prev => ({ ...prev, pharmacyImages: Array.from(e.target.files || []) }));
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const drugLicenseBase64 = files.drugLicense ? await convertFileToBase64(files.drugLicense) : null;
      const pharmacyImagesBase64 = files.pharmacyImages.length > 0 
        ? await Promise.all(files.pharmacyImages.map(file => convertFileToBase64(file)))
        : [];

      const partnerData = {
        name: formData.pharmacyName,
        type: 'health',
        description: formData.description || `Pharmacy - ${formData.medicineType || 'Medicine Store'}`,
        ...
        formData: {
          ...
          drugLicense: drugLicenseBase64,
          pharmacyImages: pharmacyImagesBase64,
        },
      };

      await api.post('/partners', partnerData);
      if (isAdmin) {
        showToast('Medicine partner added successfully!', 'success');
        // Reset form
        setFormData({
          pharmacyName: '',
          email: '',
      ...
      if (error instanceof ApiError && error.status === 401) {
        ...
        const errorMsg = error.message || 'Failed to add medicine partner. Please try again.';
        setError(errorMsg);
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="w-full">
        ...
        <div className="mb-6 px-6 pt-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Add Medicine Partner</h1>
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <Link href="/admin" className="hover:text-[#10b981]">Home</Link>
            <span>/</span>
            <span>Add Medicine Partner</span>
          </div>
        </div>

        <Card className="p-6 lg:p-8 mx-0">
      <div className="mb-6">
            <h2 className="text-2xl font-bold text-[#10b981] mb-1">Add Medicine Partner</h2>
            <p className="text-[#10b981]">Adding Medicine Partner</p>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-900 mb-2">
                    Pharmacy/Medicine Store Name <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    name="pharmacyName"
                    ...
                    placeholder="Enter pharmacy or medicine store name"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Pharmacy description and services"
                  />
                </div>
              </div>
      </div>
            ...
            <div className="bg-[#ecfdf5] border border-[#10b981] rounded-lg p-4">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {files.drugLicense ? files.drugLicense.name : 'No file chosen'}
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-900 mb-2 uppercase">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-2 uppercase">
                  Upload Pharmacy Images (Multiple)
                </label>
                <label className="cursor-pointer">
                  <span className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-700">
                    <Upload className="h-4 w-4 mr-2" />
                  ...
                  <input
                    ...
                    accept="image/*"
                    multiple
                    onChange={(e) => handleFileChange(e, 'pharmacyImages')}
                    className="hidden"
                  />
                </label>
                <p className="text-xs text-gray-500 mt-1">
                  {files.pharmacyImages.length > 0 
                    ? `${files.pharmacyImages.length} file(s) selected`
                    : 'No file chosen'}
                </p>
