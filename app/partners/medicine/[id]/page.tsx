...
import { ArrowLeft, MapPin, Phone, Mail, Clock, Map, Pill, Heart, Loader2, Ticket, X, QrCode, Copy, CheckCircle } from 'lucide-react';
...
export default function MedicinePartnerDetailPage() {
  ...
  const handleGetCoupon = async () => {
    try {
      ...
      if (!token || token.trim() === '') {
        ...
        localStorage.setItem('redirectAfterLogin', `/partners/medicine/${partnerId}`);
        router.push('/login');
        return;
      }
      ...
      const response = await api.post<any>('/coupons', {
        amount: 0,
        paymentId: `medicine-partner-${partnerId}-${Date.now()}`,
        paymentStatus: 'completed',
        beneficiaryName: partner?.name || partner?.businessName || 'Medicine Partner',
        partnerId: partnerId,
      });
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-16 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/partners/medicine')}>
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Medicine Partners
          </Button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <button
            onClick={() => router.push('/partners/medicine')}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-[5px]">
          <div className="lg:col-span-7">
            <div className="relative h-64 sm:h-80 lg:h-96 w-full overflow-hidden bg-gray-200">
              <Image
                src={partner.photo || partner.logo || 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800&h=600&fit=crop'}
              ...
              <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
                <div className="bg-white/90 backdrop-blur-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg shadow-md">
                  <Image
                    src={partner.logo || 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800&h=400&fit=crop'}
