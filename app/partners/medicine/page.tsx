...
import { Pill, Heart, Users, Award, Map, Loader2, Ticket, X, QrCode, Copy, CheckCircle } from 'lucide-react';
...
export default function MedicinePartnersPage() {
  const router = useRouter();
  const [medicinePartners, setMedicinePartners] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState({ customersServed: 0, yearsCombined: 0 });
  ...
  useEffect(() => {
    fetchMedicinePartners();
  }, []);

  const fetchMedicinePartners = async () => {
    try {
      setLoading(true);
      const data = await api.get<any[]>('/partners/type/medicine');
      if (Array.isArray(data)) {
        const formatted = data.map((partner: any) => {
          ...
          return {
            ...
            logo: partner.logo || photo || 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800&h=400&fit=crop',
            photo: photo || partner.photo || partner.logo || 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800&h=600&fit=crop',
            description: partner.description || 'Medicine/Pharmacy partner',
            ...
            operatingHours: partner.operatingHours || 'MON-SUN: 09:00 - 21:00',
        ...
        formatted.forEach((partner: any) => {
          if (partner.impact) {
            ...
            const customersMatch = impactStr.match(/([\d.]+)([mk]?)\+?/);
            if (customersMatch) {
              let customers = parseFloat(customersMatch[1]);
              const unit = customersMatch[2];
              if (unit === 'm') customers *= 1000000;
              else if (unit === 'k') customers *= 1000;
      ...
      if (error instanceof ApiError) {
        console.error('Failed to fetch medicine partners:', error.message);
      } else {
        console.error('Failed to fetch medicine partners');
      }
      setMedicinePartners([]);
    } finally {
      setLoading(false);
  ...
  const handleGetCoupon = async (partner: any) => {
    try {
      ...
      if (!token || token.trim() === '') {
        ...
        localStorage.setItem('redirectAfterLogin', '/partners/medicine');
        router.push('/login');
        return;
      }
      ...
      const response = await api.post<any>('/coupons', {
        amount: 0,
        paymentId: `medicine-partner-${partner.id}-${Date.now()}`,
        paymentStatus: 'completed',
        beneficiaryName: partner.name || 'Medicine Partner',
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-16">
          <Pill className="h-16 w-16 text-[#10b981] mx-auto mb-6" />
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Medicine/Pharmacy Partners</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Working together with leading pharmacies to provide quality medicines and healthcare products across India.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
            ...
            <div className="text-3xl font-bold mb-2">{medicinePartners.length}</div>
            <div className="text-green-100">Pharmacy Partners</div>
          </Card>
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
            <Heart className="h-10 w-10 mx-auto mb-4" />
            <div className="text-3xl font-bold mb-2">
              {stats.customersServed >= 1000000 
                ? `${(stats.customersServed / 1000000).toFixed(0)}M+`
                : stats.customersServed >= 1000
                ? `${(stats.customersServed / 1000).toFixed(0)}K+`
                : stats.customersServed > 0
                ? `${stats.customersServed.toLocaleString()}+`
                : '0'}
            </div>
            <div className="text-green-100">Customers Served</div>
          </Card>
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
        ...
        {medicinePartners.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            ...
            <Card 
              ...
              onClick={() => router.push(`/partners/medicine/${partner.id}`)}
            >
          ...
          <div className="text-center py-16">
            <p className="text-gray-500 text-lg mb-4">No medicine/pharmacy partners available yet</p>
        ...
        <Card className="p-8 bg-gradient-to-r from-[#10b981] to-[#059669] text-white text-center">
          <h2 className="text-3xl font-bold mb-4">Become a Medicine/Pharmacy Partner</h2>
          <p className="text-green-100 mb-6 text-lg">
            Join us in providing quality medicines. Partner with Care Foundation Trust® to make a difference.
          </p>
