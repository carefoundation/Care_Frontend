...
import { ClipboardList, Heart, Users, Award, Map, Loader2, Ticket, X, QrCode, Copy, CheckCircle } from 'lucide-react';
...
export default function EventFormPartnersPage() {
  const router = useRouter();
  const [eventFormPartners, setEventFormPartners] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState({ eventsOrganized: 0, yearsCombined: 0 });
  ...
  useEffect(() => {
    fetchEventFormPartners();
  }, []);

  const fetchEventFormPartners = async () => {
    try {
      setLoading(true);
      const data = await api.get<any[]>('/partners/type/event-form');
      if (Array.isArray(data)) {
        const formatted = data.map((partner: any) => {
          ...
          if (!photo && partner.formData) {
            ...
            } else if (partner.formData.eventImages && Array.isArray(partner.formData.eventImages) && partner.formData.eventImages.length > 0) {
              photo = partner.formData.eventImages[0];
            }
          }
          return {
            id: partner._id || partner.id,
            name: partner.name || partner.businessName || 'Unknown',
            logo: partner.logo || photo || 'https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=800&h=400&fit=crop',
            photo: photo || partner.photo || partner.logo || 'https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=800&h=600&fit=crop',
            description: partner.description || 'Event Form partner',
            ...
            operatingHours: partner.operatingHours || 'MON-SUN: 09:00 - 18:00',
        ...
        setEventFormPartners(formatted);
        ...
        formatted.forEach((partner: any) => {
          if (partner.impact) {
            ...
            const eventsMatch = impactStr.match(/([\d.]+)([mk]?)\+?/);
            if (eventsMatch) {
              let events = parseFloat(eventsMatch[1]);
              const unit = eventsMatch[2];
              if (unit === 'm') events *= 1000000;
              else if (unit === 'k') events *= 1000;
              totalEvents += events;
            }
          }
        ...
        setEventFormPartners([]);
      }
    } catch (error) {
      if (error instanceof ApiError) {
        console.error('Failed to fetch event form partners:', error.message);
      } else {
        console.error('Failed to fetch event form partners');
      }
      setEventFormPartners([]);
    } finally {
      setLoading(false);
  ...
  const handleGetCoupon = async (partner: any) => {
    try {
      ...
      if (!token || token.trim() === '') {
        ...
        localStorage.setItem('redirectAfterLogin', '/partners/event-form');
        router.push('/login');
        return;
      }
      ...
      const response = await api.post<any>('/coupons', {
        amount: 0,
        paymentId: `event-form-partner-${partner.id}-${Date.now()}`,
        paymentStatus: 'completed',
        beneficiaryName: partner.name || 'Event Form Partner',
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-16">
          <ClipboardList className="h-16 w-16 text-[#10b981] mx-auto mb-6" />
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Event Form Partners</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Working together with event organizers to create meaningful community events across India.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
            ...
            <div className="text-3xl font-bold mb-2">{eventFormPartners.length}</div>
            <div className="text-green-100">Event Partners</div>
          </Card>
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
            <Heart className="h-10 w-10 mx-auto mb-4" />
            <div className="text-3xl font-bold mb-2">
              {stats.eventsOrganized >= 1000000 
                ? `${(stats.eventsOrganized / 1000000).toFixed(0)}M+`
                : stats.eventsOrganized >= 1000
                ? `${(stats.eventsOrganized / 1000).toFixed(0)}K+`
                : stats.eventsOrganized > 0
                ? `${stats.eventsOrganized.toLocaleString()}+`
                : '0'}
            </div>
            <div className="text-green-100">Events Organized</div>
          </Card>
          <Card className="p-6 text-center bg-gradient-to-br from-[#10b981] to-[#059669] text-white">
        ...
        {eventFormPartners.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {eventFormPartners.map((partner) => (
            <Card 
              key={partner.id} 
              hover 
              className="overflow-hidden cursor-pointer"
              onClick={() => router.push(`/partners/event-form/${partner.id}`)}
            >
          ...
          <div className="text-center py-16">
            <p className="text-gray-500 text-lg mb-4">No event form partners available yet</p>
        ...
        <Card className="p-8 bg-gradient-to-r from-[#10b981] to-[#059669] text-white text-center">
          <h2 className="text-3xl font-bold mb-4">Become an Event Form Partner</h2>
          <p className="text-green-100 mb-6 text-lg">
            Join us in organizing community events. Partner with Care Foundation Trust® to make a difference.
          </p>
